...
class Dataset(BaseDataset):
    ...
    parameters = {
        ...
        "noise_level": [0.01],
        "seed": [42],
        "geometry_type_2d": ["parallel"],
        "detector_spacing_2d": [1.0],
        "pixel_spacing_2d": [1.0],
    }

    def prepare(self):
        check_installed("tomography_2d", config.get_data_path(key="tomography"))
        ...
        with DistributedContext(seed=self.seed, cleanup=False) as ctx:
            ...
            invprob_conf = InvProbConfig(
                ...
                params={
                    ...
                    "seed": self.seed,
                    "geometry_type_2d": self.geometry_type_2d,
                    "detector_spacing_2d": self.detector_spacing_2d,
                    "pixel_spacing_2d": self.pixel_spacing_2d,
                },
            )

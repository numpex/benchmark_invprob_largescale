...
class _LazyVolList:
    ...
    Replaces the old eager ``evn_vols`` / ``odd_vols`` lists while keeping
    the same ``ds.evn_vols[i]`` interface expected by the inference loop in
    run_ei_patch.py.  Each ``__getitem__`` call loads the full volume into RAM
